...
public class EditMavenArtifactDialog extends BaseDialog {
    ...
    private static final Pattern REGEX_FOR_GRADLE = Pattern.compile("\\s*(?:(group:\\s+'(?<group>.*?)',\\s+name:\\s+'(?<name>.*?)'(,\\s+version:\\s+'(?<version>.*?)')?(,\\s+classifier:\\s+'(?<classifier>.*?)')?)|'(?<shortGroup>.*?):(?<shortName>.*?)(:(?<shortVersion>.*?))?(:(?<shortClassifier>.*?))?')\\s*", Pattern.MULTILINE);
    ...
    private void parseArtifactText() {
        try {
            ...
            log.trace("Error parsing artifact!", e);
        }
    }
    ...
    private List<DriverLibraryMavenArtifact> parseGradle() throws DBException {
        ...
        while (matcher.find()) {
            ...
            if (!CommonUtils.isNotEmpty(group) && !CommonUtils.isNotEmpty(name)) {
                // Try to match short form instead
                group = matcher.group("shortGroup");
                name = matcher.group("shortName");
                version = matcher.group("shortVersion");
                classifier = matcher.group("shortClassifier");
            }

            if (CommonUtils.isNotEmpty(group) && CommonUtils.isNotEmpty(name)) {
                DriverLibraryMavenArtifact lib = new DriverLibraryMavenArtifact(EditMavenArtifactDialog.this.driver, DBPDriverLibrary.FileType.jar, "", CommonUtils.isNotEmpty(version) ? version : "RELEASE");
                lib.setReference(new MavenArtifactReference(CommonUtils.notEmpty(group), CommonUtils.notEmpty(name), classifier, CommonUtils.isNotEmpty(version) ? version : "RELEASE"));
                artifacts.add(lib);
            } else {
                throw new DBException("Wrong Gradle configuration!");
            }
        }
